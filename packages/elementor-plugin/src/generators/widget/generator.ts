...
function normalizeOptions(
  ...
  const pluginRoot = `${getWorkspaceLayout(host).libsDir}/${pluginDirectory}`;
  ...
  const widgetRoot = `${getWorkspaceLayout(host).libsDir}/${widgetDirectory}`;
...
export default async function (
  ...
  addProjectConfiguration(host, normalizedOptions.widgetName, {
    ...
    targets: {
      build: {
        executor: '@betrue/elementor-widget:build',
      },
    },
